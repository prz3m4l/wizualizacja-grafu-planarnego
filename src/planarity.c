      ...
      if (f->inFragment[v]) {
        if (u < v) {
          if (addVertex(sub.vertices, origToSub[u], origToSub[v]) == -1 ||
            ...
            for (int k = 0; k < subV; k++) free(sub.vertices[k].neighbours);
            free(sub.vertices);
            free(origToSub);
            return false;
          }
    ...
    for (int i = 0; i < graph->vertices[u].count; i++) {
      ...
      if (!edgeDrawn[u][i] && edgeVisited[u][i] != visitId) {
        ...
        if (!frag->vertices || !frag->contactPoints || !frag->inFragment || !frag->isContact) {
          ...
          if (frag->contactPoints) free(frag->contactPoints);
          if (frag->inFragment) free(frag->inFragment);
          if (frag->isContact) free(frag->isContact);
...
static bool embedPathAndSplitFace(Face **facesPtr, int *facesCount,
  ...
  for (int i = 0; i < pathLength - 1; i++) {
    ...
    if (idxU != -1) edgeDrawn[u][idxU] = true;
    int idxV = getNeighbourIndex(graph, v, u);
    if (idxV != -1) edgeDrawn[v][idxV] = true;
  }
  return true;
}
